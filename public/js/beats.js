// ===============================
// 🔥 BEATS.JS - NOX SECURE DOWNLOAD + AUTH MODAL + CHARTS + 3 COLS - FIXED
// ===============================
...
const DOWNLOAD_API = "https://ai-api.dopetone701.workers.dev";
...
async function noxDownload(beat, btn){
  ...
  if(!user){
    ...
    else { document.getElementById('authModal')?.classList.add('active'); document.body.style.overflow='hidden'; }
    window.Auth?.showToast?.('Sign in to download');
    return;
  }
  ...
  try{
    ...
    setTimeout(()=>{ btn.innerHTML = origHTML; btn.style.background = ''; btn.style.color = ''; btn.disabled = false; activeDL.delete(String(beat.id)); },2500);
  }catch(err){
    console.error('[NOX DL FAIL]', err);
    btn.innerHTML = `Retry`; btn.disabled = false; activeDL.delete(String(beat.id));
  }
}

const handleBuy = async (beat, btn) => {
  const mode = getMode(beat);
  if (mode === 'free') { await noxDownload(beat, btn); return; }
  let cart = JSON.parse(localStorage.getItem("dopetone_cart") || "[]");
  const cartBeat = { id: beat.id, title: beat.title, cover: beat.cover_url, cover_url: beat.cover_url, genre: beat.genre, bpm: beat.bpm, audio: beat.mp3_url || beat.audio, mp3_url: beat.mp3_url, zip_url: beat.zip_url, mood: beat.mood, type: beat.type, key: beat.key, monetization_mode: mode, price: fixPrice(beat.price) };
...
let currentBeats = []; let renderedCount = 0; const CHUNK = 24; let isLoadingChunk = false; let observer = null;

function createGridCard(beat, index, allBeats){
  const mode = getMode(beat);
  const card = document.createElement('div');
  card.className = 'featured-card grid-featured-card';
  card.dataset.mode = mode; card.dataset.index = index;
  card.innerHTML = `<div class="f-cover-wrap"><img src="${beat.cover_url || 'images/studio.jpg'}" loading="lazy" decoding="async" alt="${beat.title}"><button class="f-play grid-play" aria-label="play"><span class="f-icon">${PLAY_SVG}</span></button></div><div class="f-content"><div class="f-title">${beat.title}</div><div class="f-meta">#${beat.genre||'Trap'} • ${beat.bpm||140} BPM</div><div class="f-price">${getPriceHTML(beat)}</div><button class="f-buy ${mode==='free'?'is-free':''}">${mode==='free'?'Free Download':'Buy'}</button></div>`;
  const playBtn = card.querySelector('.f-play'); const buyBtn = card.querySelector('.f-buy');
  playBtn.onclick = (e)=>{ e.stopPropagation(); const isCurrent = window.globalPlayer?.currentIndex === index && window.__CURRENT_LIST__ === 'grid'; if(isCurrent && window.globalPlayer?.isPlaying) window.globalPlayer.pause(); else window.globalPlayer.play(index, allBeats, 'grid'); };
...
function renderNextChunk(){
  ...
  next.forEach((beat, i) => { const card = createGridCard(beat, renderedCount + i, currentBeats); frag.appendChild(card); });
  container.appendChild(frag); renderedCount += next.length; isLoadingChunk = false;
}
...
function initInfiniteObserver(){
  ...
  observer = new IntersectionObserver((entries)=>{ if(entries[0].isIntersecting) renderNextChunk(); }, { rootMargin: '800px' });
...
function renderGridView(filteredBeats = null) {
  ...
  container.innerHTML = ''; renderedCount = 0; renderNextChunk(); initInfiniteObserver();
  document.removeEventListener("playerPlay", window.__gridPlaySync__);
  window.__gridPlaySync__ = (e) => {
    const { index, listId } = e.detail||{};
    document.querySelectorAll(".grid-featured-card.f-icon").forEach((el, i) => { el.innerHTML = (listId === "grid" && i === index)? PAUSE_SVG : PLAY_SVG; });
  };
  document.addEventListener("playerPlay", window.__gridPlaySync__);
  document.addEventListener("playerPause", () => { if (window.__CURRENT_LIST__!== "grid") return; document.querySelectorAll(".grid-featured-card.f-icon").forEach(b => b.innerHTML = PLAY_SVG); });
}
...
function autoHideDropdown(){
  ...
  dropdownTimer = setTimeout(()=>{
    ...
    setTimeout(()=>{ dd.style.display='none'; dd.classList.remove('fade-out'); dd.innerHTML=''; }, 400);
  }, 3000);
}
function hideDropdownInstant(){
  const dd = document.getElementById('searchDropdown');
  if(!dd) return;
  clearTimeout(dropdownTimer);
...
document.addEventListener('DOMContentLoaded', async () => {
  if (!window.store?.loaded) {
    await new Promise(resolve => { const check = setInterval(() => { if (window.store?.loaded) { clearInterval(check); resolve(); } }, 50); });
  }
  initToggle(); initSearch(); initPills(); initSticky();
...
function initToggle() {
  const listBtn = document.getElementById("listBtn"); const gridBtn = document.getElementById("gridBtn");
  const listSection = document.getElementById("arsenalSection"); const gridSection = document.getElementById("gridSection");
  if (!listBtn ||!gridBtn) return;
  listBtn.addEventListener("click", () => { listSection.style.display = "block"; gridSection.style.display = "none"; listBtn.classList.add("active"); gridBtn.classList.remove("active"); if (!listSection.dataset.loaded) { renderWave(null); listSection.dataset.loaded = "true"; } });
  gridBtn.addEventListener("click", () => { listSection.style.display = "none"; gridSection.style.display = "block"; gridBtn.classList.add("active"); listBtn.classList.remove("active"); renderGridView(); gridSection.dataset.loaded = "true"; });
  listBtn.click();
}
...
function initPills(){
  document.querySelectorAll('.pill').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      if(genre.toLowerCase() === 'all'){ searchInput.value = ''; } else { searchInput.value = genre; }
...
function initSearch() {
  ...
  input.addEventListener('input', (e) => {
    ...
    if (!query) {
      dropdown.classList.remove('active'); hideDropdownInstant();
    ...
    if (currentResults.length > 0) { showDropdownResults(currentResults.slice(0, 6)); rerenderAllSections(currentResults); autoHideDropdown(); }
  ...
  function showDropdownResults(beats) {
    dropdown.innerHTML = '';
    beats.forEach((beat) => {
      ...
      item.onclick = () => {
        ...
        hideDropdownInstant(); // CLOSE ON SELECT
        if (window.globalPlayer) window.globalPlayer.play(0, [beat], "wave");
        rerenderAllSections([beat]);
      };
      dropdown.appendChild(item);
    });
    dropdown.classList.add('active'); dropdown.style.display='block';
  }
  function showInfiniteRespawn() {
    ...
    dropdown.classList.add('active'); dropdown.style.display='block'; setTimeout(() => hideDropdownInstant(), 1500);
  }
  // outside click + hover logic
  document.addEventListener('click', (e) => { if (!e.target.closest('.arsenal-search')) hideDropdownInstant(); });
  dropdown.addEventListener('mouseenter', ()=>{ clearTimeout(dropdownTimer); dropdown.classList.remove('fade-out'); dropdown.style.opacity='1'; });
  dropdown.addEventListener('mouseleave', ()=>{ autoHideDropdown(); });
}
...
function initSticky(){
  ...
  if(!bar ||!nav) return;
  const navH = nav.offsetHeight; const startTop = bar.getBoundingClientRect().top + window.scrollY;
  window.addEventListener('scroll', () => {
    if (window.scrollY >= startTop - navH) {
      bar.style.position = 'fixed'; bar.style.top = navH + 'px'; bar.style.left = '0'; bar.style.right = '0'; bar.style.zIndex = '1002'; bar.style.width = '100%';
    } else { bar.style.position = 'sticky'; bar.style.top = navH + 'px'; }
    if(window.scrollY > 80) bar?.classList.add('is-stuck'); else bar?.classList.remove('is-stuck');
  });
}
