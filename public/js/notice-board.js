// notice-board-v6-super.js - V6 SUPER - LLM + VECTORIZE + NO WRONG GENRE + FULLSCREEN + PST STYLE
...
if (INPUT) { INPUT.placeholder = "Try: I need dark drill 142 bpm or slime vibe 💬"; INPUT.style.fontSize="16px"; }
...
// V6 SUPER ADMIN CORRECTION - with Vectorize support
async function handleAdminCorrection(text){
  if(!isAdminUser()) return false;
  let t = text.trim();
  let lower = t.toLowerCase();

  if(lower.startsWith('correct:') || lower.startsWith('learn:') || lower.startsWith('teach:') || lower.startsWith('fix:') || lower.startsWith('forget:') || lower.startsWith('admin:')){
      ...
      if(word){
        try{
          await fetch(`${AI_API_URL}/api/correction`,{
            ...
            body: JSON.stringify({ user_text: word, genre:'', intent:'delete', response:'', by:'admin-v6-forget' })
          });
          alert(`V6 SUPER FORGET: "${word}" removed from D1 + Vectorize. Vectors deleted.`);
        }catch(e){ alert('forget error '+e.message); }
      }
      return true;
    }

    // learn: slime means drill
    let match = t.match(/when user says ["']?(.+?)["']? (?:genre is|means|show|is) (.+)/i) || t.match(/["']?(.+?)["']? (?:means|is) (.+)/i) || t.match(/learn:\s*["']?(.+?)["']?\s*means\s*(.+)/i);
    if(match){
      let trigger = match[1].toLowerCase().trim();
      let value = match[2].toLowerCase().trim();
      let genre = value.includes('trap')?'trap': value.includes('drill')?'drill': value.includes('edm')?'edm': value.includes('afro')?'afrobeat': value.includes('amapiano')?'amapiano': value.includes('r&b')||value.includes('rnb')?'r&b': value.includes('plug')?'plug': value.includes('future')?'future bass': value.includes('rage')?'rage': value.split(' ')[0];

      try{
        await fetch(`${AI_API_URL}/api/correction`,{
          method:'POST', headers:{'Content-Type':'application/json'},
          body: JSON.stringify({ user_text: trigger, intent:'need_beat', genre: genre, response: value, by:'Dopetone701-admin-v6-super' })
        });
        alert(`V6 SUPER LEARNED FOREVER: "${trigger}" -> "${genre}" saved to D1 + Vectorize (768d). Bot will never forget. Test now: type "${trigger}"`);
...
function buildLayout(){
  ...
  FEED.innerHTML=`
    ...
    <div id="dtChatWrap" style="margin-top:14px;background:#0a0a0a;border:1px solid #1e1e2e;border-radius:20px;overflow:hidden;max-height:0px;opacity:0;transform:translateY(-20px);pointer-events:none;transition:all.6s cubic-bezier(.22,1,.36,1)">
      <div style="padding:10px 14px;background:#0a0a0f;border-bottom:1px solid #1e1e2e;display:flex;align-items:center;gap:8px"><div style="width:7px;height:7px;background:#0d3bff;border-radius:50%;box-shadow:0 0 8px #0d3bff"></div><span style="color:#fff;font-size:10px;font-weight:800">LIVE CHAT • Dope Tone Creators • v6 SUPER ⚡</span><span id="dtTypingHead" style="margin-left:auto;font-size:9px;color:#0d3bff;display:none">AI thinking...</span></div>
      ...
      <div id="dtTypingIndicator" style="display:none;padding:0 14px 10px;background:rgba(0,0,0,.75)"><div style="display:flex;gap:8px;align-items:center"><div style="width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#0d3bff,#ff1a2e);display:flex;align-items:center;justify-content:center;color:#fff;font-size:7px;font-weight:800">AI</div><div style="background:#15152a;border:1px solid #0d3bff33;padding:10px 14px;border-radius:18px;color:#fff;font-size:11px">⚡ V6 SUPER thinking...</div></div></div>
    </div>
...
function appendBubble(c,isTemp=false){
  ...
  if(!isTemp && c.is_admin!=1){
    ...
    if(existing) { lastChatIds.add(c.id); return; }
  }
  ...
  if(isCreator){
    ...
    div.innerHTML=`<div style="width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,#0d3bff,#ff1a2e);display:flex;align-items:center;justify-content:center;color:#fff;font-size:7px;font-weight:800">AI</div><div style="background:linear-gradient(135deg,#15152a,#0f0f1e);border:1px solid #0d3bff44;border-left:2px solid #0d3bff;color:#e6e9ff;padding:10px 14px;border-radius:18px 18px 18px 4px;font-size:13px;white-space:pre-wrap;word-break:break-word"><div style="font-size:7px;color:#6d7bff;font-weight:800;margin-bottom:3px">V6 SUPER • ${c.source||'LLM'}</div>${escapeHtml(c.message)}<div style="font-size:9px;color:#555;margin-top:5px">${new Date(c.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</div></div>`;
  }else{
...
// V6 SUPER AI SYSTEM - PST STYLE RENDER
...
async function loadProAI(){
  ...
  try{
    const [intentMod, beatMod, greetMod, respMod, memMod] = await Promise.all([
      import('./ai-replies/intent-detector-pro.js?v6-super-'+Date.now()),
      import('./ai-replies/beat-engine-pro.js?v6-super-'+Date.now()),
      import('./ai-replies/greetings-pro.js?v6-super-'+Date.now()),
      import('./ai-replies/response-builder-pro.js?v6-super-'+Date.now()),
      import('./ai-replies/conversation-memory-pro.js?v6-super-'+Date.now())
    ]);
    proModules = { intentMod, beatMod, greetMod, respMod, memMod };
    proAIReady = true;
    console.log('DopeAI V6 SUPER modules loaded - LLM + Vectorize ⚡');
    return proModules;
  }catch(e){
...
function renderRecommendationsPro(beats, genre, fallback=false, entities={}){
  ...
  // V6 SUPER GUARD - strict genre lock
  ...
  if(askGenre && askGenre!=='recent'){
    ...
    if(filtered.length) beats = filtered;
      ...
      // Don't show wrong genre - show empty and ask clarification
    ...
    <div style="background:#121212;border:1px solid #282828;border-radius:20px;overflow:hidden">
      <div style="padding:10px 14px;background:#181818;border-bottom:1px solid #282828;display:flex;align-items:center;gap:8px;justify-content:space-between">
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <span style="font-size:11px;color:#fff;font-weight:800;letter-spacing:.5px">${beats.length} BEATS • V6 SUPER ${ (genre||'').toUpperCase() } • ${beats[0]._rank_score? 'Score '+Math.round(beats[0]._rank_score) : 'MATCH'}</span>
        </div>
...
// V6 SUPER: ASYNC LLM FIRST
async function getGiantReply(userText){
  let pro = await loadProAI();
  if(!pro){
    return "We got you. What beat you like? Select below 👇";
  }
  
  // V6 SUPER - ASYNC intent detection with LLM
  let parsed = await pro.intentMod.detectIntentPro(userText);
  console.log('[V6 SUPER] intent:', parsed.intent, 'entities:', parsed.entities, 'source:', parsed.source, 'confidence:', parsed.confidence, 'page:', pro.memMod.getMemoryPro().page);
  ...
  // If unclear / low confidence - ask, don't show wrong beats
  if(parsed.intent === 'unclear' || parsed.confidence < 65){
    let resp = await pro.respMod.buildResponsePro({intent: 'unclear', entities: parsed.entities, beatsResult: {beats:[]}});
    setTimeout(()=> renderOptionsPro(resp.options), 300);
    return resp.text;
  }

  let beatsResult = { beats:[], fallback:false };
  let shouldFetch = ['need_beat','what_we_have','pricing','next_page','buy_intent','recent'].includes(parsed.intent) || parsed.entities.genre || parsed.entities.q || parsed.entities.artist_type || parsed.entities.bpm || parsed.entities.emotional_context;

  if(shouldFetch){
    let mem = pro.memMod.getMemoryPro();
    let taste = pro.memMod.getTasteVectorPro();
    let genre = parsed.entities.genre || taste.topGenre || mem.genre || (parsed.intent==='recent'? null : null);
    ...
    beatsResult = await pro.beatMod.fetchBeatsPro({...parsed.entities, genre: genre, page: page, limit:3, intent: parsed.intent});
    ...
    if(beatsResult.beats.length){
      pro.memMod.saveMemoryPro({
        ...
        artist_type: parsed.entities.artist_type,
        emotional_context: parsed.entities.emotional_context,
    ...
    pro.memMod.saveMemoryPro({lastUserText:userText, intent:parsed.intent, genre: parsed.entities.genre});
  }

  // V6 SUPER: LLM generated reply - zero hardcoded
  let resp = await pro.respMod.buildResponsePro({intent: parsed.intent, entities: parsed.entities, beatsResult});
  setTimeout(()=> renderOptionsPro(resp.options), 300);

  // V6 SUPER LEARNING - send to D1 + Vectorize
  try{
...
async function showAutoReply(userText){
  ...
  setTimeout(()=>{
    showTyping(false);
    appendBubble({id:'auto-'+Date.now(), user_name:'Dope Tone Creators', user_id:'admin', message:msg, is_admin:1, reply_to_user_id:uid, created_at:new Date().toISOString(), source:'v6-super-llm' });
...
async function sendChat(){
  ...
  if(isAdminUser()){
    ...
    if(handled){
      INPUT.value='';
      appendBubble({id:'admin-'+Date.now(), user_name:'Admin', user_id:'admin', message:'✅ V6 SUPER Learned & saved to D1 + Vectorize forever. Test it now.', is_admin:1, created_at:new Date().toISOString()});
      return;
    }
  }
...
console.log("DopeAI NOTICE BOARD v6 SUPER - LLM + Vectorize + No Wrong Genre + Zero Hardcoded ⚡");
